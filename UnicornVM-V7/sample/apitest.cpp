...
// interpretee
static int print_message(const char *reason, const char **argv, FILE *logfp,
                         fn_vc_callback_t cb) {
...
int vrun_print_message(const char *reason, const char **argv, FILE *logfp,
                       fn_vc_callback_t cb) {
  vc_context_t ctx;
...
int wrapper_print_message(const char *reason, const char **argv, FILE *logfp,
                          fn_vc_callback_t cb) {
...
int main(int argc, const char *argv[]) {
  ...
  for (size_t i = 0; i < sizeof(testor) / sizeof(testor[0]); i++) {
    ...
    fflush(stdout);
    printf("////// result %d //////\n\n",
           testor[i].func(testor[i].reason, argv, testor[i].logfp,
                          testor[i].interpcb));
    fflush(stdout);
  }
  fclose(logfp);
